...
wavwrite(d1(1:ml)+d2(1:ml), sr, 'original.wav');
% or, in stereo
%soundsc([d1(1:ml),d2(1:ml)],sr)
wavwrite([d1(1:ml),d2(1:ml)],sr,'original_estereo.wav');
...
% Listen to the results
% Warped version alone
wavwrite(d2x, sr, 'warped.wav');
% Warped version added to original target (have to fine-tune length)
d2x = resize(d2x', length(d1),1);
wavwrite(d1+d2x, sr, 'warped_added_to_original.wav');
% .. and in stereo
%soundsc([d1,d2x],sr)
wavwrite([d1,d2x], sr, 'warped_stereo.wav');
% Compare to unwarped pair:
%soundsc([d1(1:ml),d2(1:ml)],sr)
wavwrite([d1(1:ml),d2(1:ml)], sr, 'unwarped_pair.wav');
